...
def can_find_the_first_item_from_the_database_using_id
  ...
  def title
    #Movie.find(:id => Movie.minimum('id'), :select => "title", :limit => 1)
    #Movie.where('id = MIN(id)').title
    # Find the client with primary key (id) 10.
    Movie.find(Movie.minimum('id')).title
  end
end
...
def can_update_multiple_items_at_once
  ...
  5.times do |i|
    ...
    movie = Movie.where(title: "Movie_#{i}").take
    movie.title = "A Movie"
    movie.save
  end
...
def can_destroy_a_single_item
  ...
  Movie.where(title: "That One Where the Guy Kicks Another Guy Once").take.destroy
